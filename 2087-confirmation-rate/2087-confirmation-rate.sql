...
#So we use 0 inside IFNULL to safely handle missing data and show 0.00 as the confirmation rate instead of NULL.
 #IFNULL(expression, 0)
select s.user_id , ROUND(IFNULL(sum(c.action = 'confirmed')/count(c.action) , 0.00), 2 ) as Confirmation_rate from Signups s left join Confirmations c on s.user_id = c.user_id group by s.user_id ;